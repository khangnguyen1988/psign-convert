<template>
  <app-modal name="document-comment-modal" :submit-on-enter="false" :title="title" @before-open="beforeOpen" @before-close="beforeClose"  width="30%" @on-ok="onOk">
    <app-form
      v-if="schema"
      ref="form"
      @on-submit="onSubmit"
      ...
      this.$refs.form.validate().then((isValid) => {
        if (isValid) {
          new DocumentProxy().createComment({
            ...
            this.$modal.hide('document-comment-modal', { isOk: true })
            this.$bus.emit('document-comment-success', response)
          })
        }
      })
    },
