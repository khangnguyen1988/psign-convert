...
The only thing you need to do is change `VUE_APP_API_URL` environment variable.
...
### Deploy ###
``` bash
$ docker login -u kakakhang -p Cnxxxx51
```
``` bash
$ ssh-copy-id root@14.225.239.83
```

 - Deploy Prod: ./deploy.sh

 - Deploy Testing: ./deploy.sh testing
